...
const NavBar = () => {
  ...
  return (
    <nav className='p-5 bg-light-orange flex justify-between items-center fixed w-full z-10 xl:text-md'>
      ...
      <ul className='hidden md:flex md:self-end'>
        {/* Iterates through the Sections array */}
        {sections.map((sections) => (
          <li
            key={sections.id}
            className='px-6 cursor-pointer text-1xl capitalize font-medium hover:scale-110 duration-300 hover:text-blue'
          >
            {/* Smooth scroll */}
            <Link to={sections.section} smooth duration={500} offset={-100}>
              {sections.section}
            </Link>
          </li>
        ))}
